...
{
public:
	file_socket(i_server& server_) : st_server_socket(server_), state(TRANS_IDLE), file(nullptr) {}
...
private:
	...
	void read_file_handler(__off64_t length)
	{
		if (nullptr != file && length > 0)
		{
			...
			{
				auto read_size = (size_t) std::min((__off64_t) packer::get_max_msg_size(), (__off64_t) ORDER_LEN + length);
				read_size -= ORDER_LEN;
