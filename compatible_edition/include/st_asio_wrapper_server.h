...
public:
	...
	void free_client(size_t num = -1)
	{
		...
		time_t now = time(NULL) - 5; //five seconds, hard coding
		mutex::scoped_lock lock(temp_client_can_mutex);
...
#ifdef AUTO_CLEAR_CLOSED_SOCKET
			{
				BOOST_TYPEOF(client_can) clients;
...
protected:
	...
	{
		BOOST_AUTO(client_ptr, do_create_client());
	...
	{
		BOOST_AUTO(client_ptr, reuse_socket());
