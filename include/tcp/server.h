...
public:
	...
	virtual bool del_socket(const boost::shared_ptr<executor>& socket_ptr)
	{
	...
	virtual bool restore_socket(const boost::shared_ptr<executor>& socket_ptr, boost::uint_fast64_t id)
	{
		BOOST_AUTO(raw_socket_ptr, boost::dynamic_pointer_cast<Socket>(socket_ptr));
		if (!raw_socket_ptr)
			return false;
