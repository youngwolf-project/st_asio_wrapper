...
{ \
	do_something_to_all([&](decltype(*std::begin(client_can))& item) { \
		SEND_AND_WAIT(item, SEND_FUNNAME); \
	}); \
} \
SEND_MSG_CALL_SWITCH(FUNNAME, void)
//msg sending interface
...
public:
	...
	uint64_t get_total_recv_bytes() const
	{
		...
		do_something_to_all([&](decltype(*std::begin(client_can))& item) {
			total_recv_bytes += dynamic_cast<test_socket*>(item.get())->get_recv_bytes();
		});
...
int main(int argc, const char* argv[])
{
	...
	{
		auto client_ptr(boost::make_shared<test_client::test_socket>(boost::ref(client)));
